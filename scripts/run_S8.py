...
primality test of 6N-1, 6N+1 (interval sieve, no probabilistic step), computing per node:
  ...
  - E(N) = prod_{distinct p|N, p>3} p/(p-2)         (per-factor amplification of the Hardy-Littlewood singular series)
...
# ------------------------------- config -------------------------------------
MAX_K     = int(os.environ.get("MAX_K", 8))   # deepest shell S_{MAX_K} (this script defaults to S8)
SEGMENT   = int(os.environ.get("SEGMENT", 2_000_000))# segment size in N (memory/speed trade-off)
OUTDIR    = os.environ.get("OUTDIR", "./recompute_out_S8")
    ...
    with open(f"{OUTDIR}/table5_enrichment_test.csv", "w", newline="") as f:
        ...
        for K in range(1, MAX_K + 1):
            ...
            if tot_sumE <= 0 or tot_twin == 0:
                continue
            baseline = tot_twin / tot_sumE         # twin rate per unit E
            for ww in range(0, OW_MAX + 1):
                nn = int(et_nodes[K, ww])
                if nn < 50:                        # sample too small to report
                    continue
                tt = int(et_twin[K, ww])
