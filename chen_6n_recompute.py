...
素性判定（区间筛，无概率成分），逐节点计算：
  ...
  - E_old(N) = ∏_{相异 p|N, p>3} p/(p-2)            ← 仅"保证存活"项（一阶近似）
  - E_new(N) = ∏_{相异 p|N, p>3} (p-1)/(p-3)        ← 含亏损修正的正确局部因子
...
# ------------------------------- 配置 ---------------------------------------
MAX_K     = int(os.environ.get("MAX_K", 8))          # 最深壳层 S_{MAX_K}
...
def factor_segment(n_lo, n_hi):
    ...
    enrich    = np.ones(size, dtype=np.float64)   # 旧因子 ∏ p/(p-2)
    enrich2   = np.ones(size, dtype=np.float64)   # 正确因子 ∏ (p-1)/(p-3)
...
def main():
    ...
    et_sumE  = np.zeros((NS, OW_MAX + 1), dtype=np.float64)   # 旧因子之和
    et_sumE2 = np.zeros((NS, OW_MAX + 1), dtype=np.float64)   # 正确因子之和
